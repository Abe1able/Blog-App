...
RSpec.describe Post, type: :model do
  before :each do
    ...
    @post = Post.new(title: 'Hi', text: 'My world', comments_counter: 0, likes_counter_integer: 0, user_id: 1)
  end
  ...
  it 'checks if user id is present' do
    @post.user_id = nil
    expect(@post).to_not be_valid
  end
  ...
  it 'checks if likes counter is an integer' do
    @post.likes_counter_integer = 'hello'
    expect(@post).to_not be_valid
  end

  describe 'Check methods in post' do
    before do
      @post1 = Post.create(title: 'Hi', text: 'My world', comments_counter: 0, likes_counter_integer: 0, user_id: 1)
      6.times do
